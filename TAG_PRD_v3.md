# TAG — Product Requirements Document (v3)
...
**Changes from v2:** adds the Platform Admin persona (P6) and an RBAC requirement (PR-1, §10), ahead of Phase 1 spec work — the platform needs an internal role that approves vendors/producers/affiliates onto the marketplace before any of P3/P4/P5's journeys can start. No other section changed; v2 is retained unedited as the prior pinned version.
...
### P6 — Platform Admin (internal, tertiary)
- TAG operations staff, not a Festival Goer or a Festival Client/Supplier — an internal role that runs the platform rather than transacting on it.
- Reviews and approves/rejects vendor (P3), producer (P4), and local-affiliate (P5) applications before they can list, sell, or operate on the platform.
- Does **not** manage or moderate Festival Goers (P1/P2) — end-user account actions stay out of this role's scope; see PR-1 below for the boundary and community moderation stays with Open Question 7's [TBD] answer.
- Wants: a queue of pending applications with the evidence needed to approve/reject (business registration, insurance, past-festival history where available), an audit trail of who approved what and when, and the ability to suspend a vendor/producer/affiliate already live on the platform (e.g. compliance breach) without needing engineering support.
...
### J7 — Platform Admin: approve vendor / producer / affiliate
```
Application submitted (J4/J5/J6's onboarding step) → appears in admin approval
queue → admin reviews evidence (registration, insurance, history) → approve
(unblocks J4/J5/J6's next step) or reject (with reason, applicant notified) →
audit log entry recorded → [post-launch] admin can suspend an already-approved
account
```
This is the gate inside J4's "verification", J5's "onboarding", and J6's
"partner onboarding" steps — those journeys don't proceed past that point
without a J7 approval. Festival Goer (P1/P2) accounts never pass through this
gate; the admin role does not action end-user accounts (see P6, §5).
...
**PR-1 — Role-based access control (P0).** The platform distinguishes at
least four role classes: Festival Goer (P1/P2, unauthenticated/guest and
authenticated), Festival Client & Supplier (P3 vendor / P4 producer / P5
affiliate — each scoped to their own festival/stall/listing data, never
another operator's), and Platform Admin (P6 — approves/rejects/suspends
Client & Supplier accounts per J7; does not action Festival Goer accounts).
Acceptance criteria: an authenticated request is authorised against the
caller's role before any write; a P3/P4/P5 account cannot read or write
another operator's data; only P6 can approve, reject, or suspend a
Client/Supplier account; every approve/reject/suspend action is
audit-logged with actor, target, timestamp, and reason. Role model detail
(permission-per-resource matrix, whether P6 itself has sub-roles) is
Phase-1-spec-level, not PRD-level.

**P0 (v1 cannot ship without):** event catalogue + festival pages; ticket purchase with seat/zone maps; cart + guest checkout + secure payments with fee transparency; virtual queue; order confirmation with QR tickets; GDPR consent management; basic accommodation + transport add-on booking via partner APIs; producer free-tier event setup; core analytics; PR-1 (RBAC, including Platform Admin vendor/producer approval).
...
9. Who holds Platform Admin (P6) accounts at launch — Nitish directly, or a first ops hire — and what's the approval-turnaround SLA fans and vendors are told to expect? (Ops)
...
11. Platform Admin (P6) staffing and approval-turnaround SLA.
